...
const AirtightnessSection: React.FC = () => {
  ...
  // Material images for airtightness with translated alt text
  const materialImages = [
    { src: fabric, altKey: 'details.airtightness.materialImages.fabric' },
    { src: plasticSheeting, altKey: 'details.airtightness.materialImages.plastic' },
    { src: sealingTape, altKey: 'details.airtightness.materialImages.tape' },
    { src: rope, altKey: 'details.airtightness.materialImages.rubber' }
  ];

  return (
    <section id="airtightness" className="scroll-mt-20">
      {/* Full-width Title & Description */}
      <div className="mb-16">
        <h2 className="text-4xl text-center font-bold mb-6 text-black dark:text-white">
          {t('details.airtightness.title')}
        </h2>
        ...
        <p className="text-lg text-center text-gray-700 dark:text-gray-300 leading-relaxed max-w-4xl mx-auto">
          {t('details.airtightness.description')}
        </p>
      </div>

      {/* Two Column Layout - Clean Background Boxes */}
      <div className="grid lg:grid-cols-2 gap-8 mb-16">
        {/* LEFT COLUMN - Simple Ways to Improve */}
        <div className="bg-gradient-to-br from-lime-50 to-green-50 dark:from-lime-950/20 dark:to-green-950/20 rounded-xl p-8 border border-lime-200 dark:border-lime-900">
          <h3 className="text-xl font-bold mb-6 text-black dark:text-white">
            {t('details.airtightness.methodsTitle')}
          </h3>

          <ul className="list-disc list-inside space-y-4">
            {(t('details.airtightness.methods', { returnObjects: true }) as string[]).map((item, index) => (
        ...
        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 rounded-xl p-8 border border-blue-200 dark:border-blue-900">
          <h3 className="text-xl font-bold mb-6 text-black dark:text-white">
            {t('details.airtightness.whyMattersTitle')}
          </h3>

          <ul className="list-disc list-inside space-y-4">
            {(t('details.airtightness.whyMatters', { returnObjects: true }) as string[]).map((item, index) => (
              <li key={index} className="text-gray-700 dark:text-gray-300 leading-relaxed">
                {item}
              </li>
            ))}
      ...
      <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-8 border border-gray-200 dark:border-slate-800">
        <h3 className="text-xl font-bold mb-8 text-center text-black dark:text-white">
          {t('details.airtightness.materialsTitle')}
        </h3>
        ...
        <p className="text-center text-gray-600 dark:text-gray-400 italic max-w-3xl mx-auto">
          {t('details.airtightness.conclusion')}
        </p>
      </div>
    </section>
  );
...
export default AirtightnessSection;